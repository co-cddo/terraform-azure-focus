locals {
  ...
  names = {
    ...
    service_plan                = coalesce(var.custom_resource_names.service_plan, "asp-cost-export-${random_string.unique.result}")
    ...
    cost_export_prefix          = coalesce(var.custom_resource_names.cost_export_prefix, "focus-daily-cost-export-${random_string.unique.result}")
    ...
    pe_deployment_blob = coalesce(local.pe_overrides.deployment_blob, "pe-storage-cost-export-deployment-${random_string.unique.result}")
    pe_function_app    = coalesce(local.pe_overrides.function_app, "pe-func-cost-export-${random_string.unique.result}")

    psc_storage_blob    = coalesce(local.psc_overrides.storage_blob, "psc-storage-cost-export-${random_string.unique.result}")
    psc_storage_queue   = coalesce(local.psc_overrides.storage_queue, "psc-storage-queue-cost-export-${random_string.unique.result}")
    psc_deployment_blob = coalesce(local.psc_overrides.deployment_blob, "psc-storage-cost-export-deployment-${random_string.unique.result}")
    psc_function_app    = coalesce(local.psc_overrides.function_app, "psc-func-cost-export-${random_string.unique.result}")

    diag_cost_export_blob  = coalesce(local.diag_overrides.cost_export_blob, "diag-blob-${random_string.unique.result}")
    diag_cost_export_queue = coalesce(local.diag_overrides.cost_export_queue, "diag-queue-${random_string.unique.result}")
    diag_deployment_blob   = coalesce(local.diag_overrides.deployment_blob, "diag-blob-deployment-${random_string.unique.result}")
